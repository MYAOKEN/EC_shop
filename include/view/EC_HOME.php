...
?>
 ...
 <p class="err-msg"><?php print $comment; ?></p>
    ...
    <div class="img_rap_top col-xs-12 col-lg-3">
				<ul class="item-list">
					<li>
						<div class="item">
							<form name="go_name" action=<?php if(@$user_name == NULL){?>"item.php"<?php }else{?> "item_user.php" <?php }?> method="post">
								<input type="image"
									src="./item_img/<?php print htmlspecialchars($value['img'], ENT_QUOTES, 'UTF-8'); ?>"
									class="sub_btn img_dev">
								<div class="item-info">
                      ...
                      <?php if($value['stock'] > 0){ ?>
                        <span class="in_stock">在庫数有</span>
                      <?php } else { ?>
                        <span class="out_stock">売り切れ</span>
                      <?php } ?>
                      </p>
                    </div>
